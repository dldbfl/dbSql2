...
-- col --> row --> table
-- TYPE 테이블 타입명 IS TABLE OF ROWTYPE/RECORD INDEX BY 인덱스 타입
-- java와 다르게 plsql에서는 array 역할을 하는 table type의 인덱스를
-- 숫자 뿐만 아니라 , 문자열 형태도 가능하다
-- 그렇기 떄문에 index에 대한 타입을 명시한다.
...
-- 1~ 10 : 55
-- 1~ 10까지의 loop를 이용하여 계산 , 결과를 s_val 이라는 변수에 담에
